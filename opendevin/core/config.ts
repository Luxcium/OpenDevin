...
import { platform, getuid } from 'node:os';
...
class SandboxConfig implements ISandboxConfig {
  ...
  userId: number = getuid ? getuid() : 1000;
  timeout: number = 120;
...
function loadFromEnv(cfg: AppConfig, envOrTomlDict: Record<string, any>) {
  ...
  function setAttrFromEnv(subConfig: any, prefix = '') {
    for (const [fieldName, fieldType] of Object.entries(subConfig)) {
...
function loadFromToml(cfg: AppConfig, tomlFile: string = 'config.toml') {
  try {
    const tomlContents = parseToml(fs.readFileSync(tomlFile, 'utf-8'));
...
function getParser(): ArgumentParser {
 ...
 default: 'evaluation/evaluation_outputs/outputs', type: 'str', help: 'The directory to save evaluation output' });
